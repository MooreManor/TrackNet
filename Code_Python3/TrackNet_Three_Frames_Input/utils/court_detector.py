...
class CourtDetector:
  ...
  def detect(self, frame, verbose=0):
    ...
    print(f"filter_pixels: {time.time()-start}s")

    # Detect lines using Hough transform
    start = time.time()
    horizontal_lines, vertical_lines = self._detect_lines(filtered)
    print(f"detect_lines: {time.time()-start}s")
    # for i in range(0, len(horizontal_lines), 1):
    #     x1, y1, x2, y2 = horizontal_lines[i][0], horizontal_lines[i][1], horizontal_lines[i][2], horizontal_lines[i][3]
    #     cv2.line(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 5)
    # for i in range(0, len(vertical_lines), 1):
    #     x1, y1, x2, y2 = vertical_lines[i][0], vertical_lines[i][1], vertical_lines[i][2], vertical_lines[i][3]
    #     cv2.line(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 5)
    # from matplotlib import pyplot as plt
    # plt.imshow(frame)
    # plt.show()
    ...
    court_warp_matrix, game_warp_matrix, self.court_score = self._find_homography(horizontal_lines,
                                                                                  vertical_lines)
    print(f"_find_homography: {time.time()-start}s")
